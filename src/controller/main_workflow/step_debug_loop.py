...
def run_debug_loop(config, project_info, mr):
    ...
    def try_fix_with_multiple_models(trace, source_code, debug_idx):
        """
        使用多个模型尝试修复代码，直到成功或所有模型都失败
        
        Returns:
            tuple: (success: bool, commit_note: str)
        """
        logger.info("开始多模型修复尝试")
        print("🔧 开始多模型修复尝试...", flush=True)
        
        # 重置模型索引到第一个模型
        llm_ctrl.reset_model_index()
        ...
        model_attempt = 0
        max_models = len(llm_ctrl.available_models)
        
        while model_attempt < max_models:
            model_attempt += 1
            current_model = llm_ctrl.get_current_model()
            ...
            logger.info(f"尝试模型 {model_attempt}/{max_models}: {current_model}")
            print(f"🤖 尝试模型 {model_attempt}/{max_models}: {current_model}", flush=True)
            
            try:
                # 5. 调用 LLM 流式 API 修复代码
                print("🛠️ 开始AI代码修复...", flush=True)
                print("🤖 正在连接AI服务端...", flush=True)
                print(f"🤖 调用模型 {current_model} 进行代码修复...", flush=True)
                
                fixed_code = llm_ctrl.fix_code_with_llm_stream(prompt)
                
                if not fixed_code or not fixed_code.strip():
                    print(f"❌ 模型 {current_model} 返回空的修复代码", flush=True)
                    logger.error(f"模型 {current_model} 返回空的修复代码")
                    
                    # 尝试下一个模型
                    if llm_ctrl.switch_to_next_model():
                        continue
                    else:
                        break
                
                print(f"\n🤖 模型 {current_model} 返回的修复代码 (长度: {len(fixed_code)}):")
                print("=" * 80)
                print("📝 Feedback Code:")
                print(fixed_code)
                print("=" * 80)
                ...
                try:
                    ...
                    if apply_success:
                        # 代码应用成功
                        logger.info(f"模型 {current_model} 的修复代码应用成功")
                        print(f"✅ 模型 {current_model} 的修复代码应用成功", flush=True)
                        
                        # 7. 提取步骤信息作为commit消息
                        step_lines = [line for line in executor_output_lines if "Step [" in line]
                        if step_lines:
                            commit_note = f"LLM auto fix using {current_model} - 调试循环第{debug_idx + 1}次修复\n\n" + "\n".join(step_lines)
                        ...
                        return True, commit_note
                    else:
                        # 代码应用失败，尝试下一个模型
                        logger.warning(f"模型 {current_model} 的代码应用失败")
                        print(f"❌ 模型 {current_model} 的代码应用失败", flush=True)
                        
                        # 尝试下一个模型
                        if llm_ctrl.switch_to_next_model():
                            print(f"🔄 尝试下一个模型...", flush=True)
                            continue
                        else:
                            break
                            
                except SystemExit:
                    # CodeFileExecutorLib 内部错误时按要求直接退出
                    logger.error(f"模型 {current_model} 的 CodeFileExecutorLib 执行出现系统退出")
                    print(f"🚨 模型 {current_model} 的 CodeFileExecutorLib 执行出现系统退出", flush=True)
                    ...
                    if llm_ctrl.switch_to_next_model():
                        ...
                        print("🚨 所有模型都失败，程序退出", flush=True)
                        raise  # 只有所有模型都失败时才退出
                        
                except Exception as e:
                    logger.error(f"模型 {current_model} 应用修复代码异常: {e}")
                    print(f"❌ 模型 {current_model} 应用修复代码异常: {e}", flush=True)
                ...
                logger.error(f"模型 {current_model} 修复代码过程异常: {e}")
                print(f"❌ 模型 {current_model} 修复代码过程异常: {e}", flush=True)
        ...
        # 所有模型都失败了
        logger.error(f"所有 {max_models} 个模型都无法成功修复代码")
        print(f"❌ 所有 {max_models} 个模型都无法成功修复代码", flush=True)
        return False, ""
    ...
    def loop_body(debug_idx):
        ...
        if status == "failed":
            ...
            # 4. 使用多模型尝试修复
            fix_success, commit_note = try_fix_with_multiple_models(trace, source_code, debug_idx)
            
            if not fix_success:
                logger.error("所有模型都无法成功修复代码，调试循环失败")
                print("❌ 所有模型都无法成功修复代码，调试循环失败", flush=True)
                return False
            ...
            try:
                ...
                current_model = llm_ctrl.get_current_model() or "UnknownModel"
                mr_title = f"LLM Auto Merge ai->dev [Fix-{debug_idx + 1}-{current_model}-{now}]"
